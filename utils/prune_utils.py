...
def get_input_mask_ori(module_defs, idx, CBLidx2mask):
    ...
    if module_defs[idx - 1]['type'] == 'convolutional':
        ...
        if module_defs[idx - 2]['type'] == 'route':  #v4 tiny
            return get_input_mask_ori(module_defs, idx - 1, CBLidx2mask)
        else:
            return CBLidx2mask[idx - 2]  #v3 tiny

def init_weights_from_loose_model_ori(compact_model, loose_model, CBL_idx, Conv_idx, CBLidx2mask):
    ...
    for idx in CBL_idx:
        ...
        input_mask = get_input_mask_ori(loose_model.module_defs, idx, CBLidx2mask)
...
def init_weights_from_loose_model(compact_model, loose_model, CBL_idx, Conv_idx, CBLidx2mask):
    ...
    for idx in CBL_idx:
        ...
        loose_CBL = loose_model.module_list[idx]
        out_channel_idx = np.argwhere(CBLidx2mask[idx])[:, 0].tolist()
        compact_bn, loose_bn         = compact_CBL[1] if type(compact_CBL[1]).__name__ is 'BatchNorm2d' else compact_CBL[0], loose_CBL[1] if type(loose_CBL[1]).__name__ is 'BatchNorm2d' else loose_CBL[0]
        compact_bn.weight.data       = loose_bn.weight.data[out_channel_idx].clone()
        compact_bn.bias.data         = loose_bn.bias.data[out_channel_idx].clone()
        compact_bn.running_mean.data = loose_bn.running_mean.data[out_channel_idx].clone()
        compact_bn.running_var.data  = loose_bn.running_var.data[out_channel_idx].clone()
        compact_bn.eps               = loose_bn.eps
        compact_bn.momentum          = loose_bn.momentum
        ...
        if isinstance(input_mask, list):
            ...
            in_channel_idx = np.argwhere(input_mask)[:, 0].tolist()
        if type(compact_CBL[0]).__name__ is 'BatchNorm2d':
        ...
        if model_def['type'] == 'convolutional':
            ...
            if i in prune_idx:
                ...
                if model_def['activation'] == 'leaky':
                    ...
                    activation=(1 - mask) * bn_module.bias.data.mul(torch.sigmoid(bn_module.bias.data))
